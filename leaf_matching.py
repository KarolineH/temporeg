...
def get_score_across_dataset(centroids, centroid_labels, outline_data, outline_labels, PCAH, components=50, add_inf=None, trim_missing=True, plotting=False, time_gap=1):
    ...
    using both our method (outline shape) and location only (centroid), as well as other transform info (add_inf).
    When trim_missing is active, leaves that are present before but not later are removed from the assignment process.
    Returns scores of all 3 methods and can also plot the individual matching results of the Bonn (location)method.
    '''

    # First check if additonal info was given, else ignore
    ...
    # We also want to compare cost distribution along true edges (whether the pairing was found or not, so true positives + false negatives),
    # and false edges (incl. false positives and true negatives).
    bonn_cost_distr=[[],[]]
    outline_cost_distr=[[],[]]
    add_inf_cost_distr=[[],[]]
    ...
    for plant in np.unique(outline_labels[:,0]): # for each plant
        for time_step in range(0,np.unique(outline_labels[:,1]).size-time_gap,1): # and for each time step available in the processed data
            ...
            if c_after.size == 0 or c_before.size == 0:
                ...
                # Optionally, remove leaves that exist only in the LATER scan. The method assumes that each leaf will be present in the next time step
                ...
                if plotting:
                    ...
                    plot_bonn_assignment(c_before, c_after, c_before_labels, c_after_labels, c_matches, o_before, o_after, title=plant_id)
                ...
                true_pair_matrix = c_before_labels[:,-1].reshape(-1,1) == c_after_labels[:,-1]
                #true_indeces = [np.where(c_before_labels[:,-1]==x)[0] if np.where(c_before_labels[:,-1]==x)[0].size >0 else None for x in c_after_labels[:,-1]]
                for dist_mat, distributions in zip([centroid_dist, outline_dist, add_inf_dist], [bonn_cost_distr, outline_cost_distr, add_inf_cost_distr]):
                    if dist_mat is not None:
                        distributions[0].append(dist_mat[np.where(true_pair_matrix==True)])
                        distributions[1].append(dist_mat[np.where(true_pair_matrix==False)])
...
def testing_pipeline(location=False, rotation=False, scale=False, as_features=False, standardise = True, trim_missing=True, components=50, time_gap=1, train_split=0, random_split=True):
    ...
    directory = os.path.join('/home', 'karolineheiwolt','workspace', 'data', 'Pheno4D', '_processed', 'pca_input') #'pca_input_upright' for only correctly aligned leaves
    ...
    bonn_scores, outline_scores, add_inf_scores, nr_of_leaves, pairings_calculated, total_true_pairings, b_cost_distr, o_cost_distr, ai_cost_distr = get_score_across_dataset(centroids, centroid_labels, test_ds, test_labels, PCAH, components=components, add_inf=additional_info, trim_missing=trim_missing, plotting=True, time_gap=time_gap)

    return bonn_scores, outline_scores, add_inf_scores, nr_of_leaves, pairings_calculated, total_true_pairings, b_cost_distr, o_cost_distr, ai_cost_distr

def hypothesis_testing(centroid, outline, add_inf):

    def means_test_statistic(x, y):
        return np.mean(x) - np.mean(y)

    results = []

    for method in [centroid, outline, add_inf]:
        if len(method[0]) > 0 and len(method[1]) > 0:
            same = np.hstack(method[0])
            different = np.hstack(method[1])

            # Permutation test
            test_res = permutation_test((same, different), means_test_statistic, n_resamples=10000, alternative='less')

            # t-test
            t_res = ttest_ind(same, different)

            # cohens d
            pooled_sd = np.sqrt(((same.shape[0] - 1) * np.std(same, ddof=1)**2 + (different.shape[0] - 1) * np.std(different, ddof=1)**2) / (same.shape[0] + different.shape[0] - 2))
            u1, u2 = np.mean(same), np.mean(different)
            cohens_d = (u1 - u2) / pooled_sd

            # https://www.frontiersin.org/articles/10.3389/fpsyg.2019.01089/full#note1a
            bins=np.histogram(np.hstack((same,different)), bins=50)[1] #get shared bin edges
            same_freq = plt.hist(same, bins=bins, alpha=1)
            diff_freq = plt.hist(different, bins=bins, alpha=0.5)
            #plt.show()
            overlap = np.minimum(np.asarray(same_freq[0]),np.asarray(diff_freq[0]))
            eta = sum(overlap) / np.min((same.shape[0],different.shape[0]))

            out = np.array([test_res.pvalue, t_res.pvalue, cohens_d, eta])
            results.append(out)
...
def tests():
    ...
    ''' Alternative method we tested: providing scale location and rotation as an additional feature at the end of the outline
    coordinate vector, before PCA. This is also where the extended baseline method is tested.'''
    ...
    ''' Main method reported in the paper: Additional information is given intrinsic to the coordinate vector'''
    ...
    # Parameters are components, time_gap, % of data used for testing only, and whether the train/test split is random
    parameters.append((23, 1, 0.754, False))
    details.append(f'Routine {j}: 22 components, using each annotated time step, training and testing on the full data set')
    j+=1
    ...
    for nr, routine, detail in zip(range(len(parameters)), parameters, details):
        ...
        for test, descr in zip(conditions, descriptions):
            print('Running ' + descr)
            bonn_scores, outline_scores, add_inf_scores, nr_of_leaves, pairings_calculated, total_true_pairings,  b_cost_distr, o_cost_distr, ai_cost_distr = testing_pipeline( \
                    location=test[0], rotation=test[1], scale=test[2], as_features=test[3], standardise=True, trim_missing=False, \
                    components=routine[0], time_gap=routine[1], train_split=routine[2], random_split=routine[3])
            # scores are x, fp, o, x_distances, fp_distances, o distances for EACH time step individually

            # fig, axs = plt.subplots(2, 1, tight_layout=True)
            # axs[0].hist(np.asarray([item for sublist in outline_scores[3] for item in sublist]), bins = 70)
            # axs[0].hist(np.asarray([item for sublist in outline_scores[4] for item in sublist]), bins = 70)
            # axs[0].hist(np.asarray([item for sublist in outline_scores[5] for item in sublist]), bins = 70)
            #
            # axs[1].hist(np.asarray([item for sublist in bonn_scores[3] for item in sublist]), bins = 70)
            # axs[1].hist(np.asarray([item for sublist in bonn_scores[4] for item in sublist]), bins = 70)
            # axs[1].hist(np.asarray([item for sublist in bonn_scores[5] for item in sublist]), bins = 70)
            # plt.show()

            # now get summed up true matches, false positives, open matches, followed by mean cost of each category, and the average % of correct correspondences across pairs:
            ...
            testing_results = hypothesis_testing(b_cost_distr, o_cost_distr, ai_cost_distr) # Bonn, outline, add_inf
            hypothesis_tests.append(np.asarray(testing_results))

            bonn_results.append(bonn_stats)
            outline_results.append(outline_stats)
            if add_inf_stats is not None:
        ...
        routine_results.append([bonn_results, outline_results, addinf_results, sum(nr_of_leaves), sum(pairings_calculated), sum(total_true_pairings), np.asarray(hypothesis_tests), detail])
    import pdb; pdb.set_trace()
...
def plot_bonn_assignment(before, after, before_labels, after_labels, legible_matches, outline_before, outline_after, title=None):
    ...
    before_centroids = ax.scatter(xs=before[:,0], ys=before[:,1], zs=before[:,2], c='lightgrey', marker='o') #c=colours[before_labels[:,-1]-1,:]
    after_centroids = ax.scatter(xs=after[:,0], ys=after[:,1], zs=after[:,2], c='grey', marker='+') # colours[after_labels[:,-1]-1,:]
    ...
    before_leaves = leaf_encoding.reshape_coordinates_and_additional_features(outline_before)[0]
    after_leaves = leaf_encoding.reshape_coordinates_and_additional_features(outline_after)[0]
    for leaf in before_leaves:
        bl= ax.scatter(xs=leaf[:,0], ys=leaf[:,1], zs=leaf[:,2], c='lightgrey')
    for leaf in after_leaves:
        al = ax.scatter(xs=leaf[:,0], ys=leaf[:,1], zs=leaf[:,2], c='grey')
...
if __name__== "__main__":
    ...
    bonn_scores, outline_scores, add_inf_scores, nr_of_leaves, pairings_calculated, total_true_pairings = testing_pipeline(location=True, \
            rotation=True, scale=True, as_features=False, standardise = True, trim_missing=False, components=23, time_gap=1, \
            train_split=0.89, random_split=False)
